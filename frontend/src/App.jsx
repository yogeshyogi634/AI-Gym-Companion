...
import { askGemini, askGeminiWithImage, fetchAllMeals, saveMeal, deleteMeal as apiDeleteMeal, fetchAllWorkouts, saveWorkoutDone, fetchWeightHistory, fetchWeight, saveWeight, fetchWater, saveWater, fetchGoals, saveGoals } from "./api";
import { getWorkoutForDate } from "./data/workouts";

import "./index.css";

const DEFAULT_GOALS = { calories:2500, protein:150, carbs:300, fat:80, fiber:35, waterGlasses:16 };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetchGoals().then(data => {
      if (data && data.calories) {
        const g = { calories: data.calories, protein: data.protein, carbs: data.carbs, fat: data.fat, fiber: data.fiber, waterGlasses: data.waterGlasses || 8 };
        setGoals(g);
        setGoalDraft(g);
      }
    });
  ...
  function logWeight() {
    ...
    setWeightHistory(prev => {
      ...
      return [{ date: dk, weight: w }, ...filtered].sort((a,b) => b.date.localeCompare(a.date));
    });
  }
  ...
  // Dashboard stats
  const waterPct = goals.waterGlasses > 0 ? Math.min((water / goals.waterGlasses) * 100, 100) : 0;
  const calPct = goals.calories > 0 ? Math.min((macros.calories / goals.calories) * 100, 100) : 0;
  ...
  return (
    <div className="h-screen overflow-hidden transition-colors duration-300 flex flex-col" style={{ background:"var(--bg-page)", color:"var(--text)", fontFamily:"'DM Sans',sans-serif" }}>
        ...
        {tab==="dashboard" && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {/* Calories Card */}
              <div className="p-4 rounded-2xl" style={{background:"var(--card-alt)", border:"1px solid var(--border)"}}>
                <div className="flex items-center gap-2 mb-2">
                  <I.Fire/>
                  <span className="text-[10px] uppercase tracking-wider font-medium" style={{opacity:"var(--dim)"}}>Calories</span>
                </div>
                <div className="text-2xl font-bold tabular-nums">{Math.round(macros.calories)}</div>
                <div className="h-1.5 rounded-full mt-2 overflow-hidden" style={{background:"var(--progress-track)"}}>
                  <div className="h-full rounded-full bg-amber-500 transition-all duration-700" style={{width:`${calPct}%`}}/>
                </div>
                <div className="text-[10px] mt-1 tabular-nums" style={{opacity:"var(--faint)"}}>{Math.round(calPct)}% of {goals.calories} kcal</div>
              </div>
              ...
              <div className="p-4 rounded-2xl" style={{background:"var(--card-alt)", border:"1px solid var(--border)"}}>
                <div className="flex items-center gap-2 mb-2">
                  <I.Dumbbell/>
                  <span className="text-[10px] uppercase tracking-wider font-medium" style={{opacity:"var(--dim)"}}>Workout</span>
                </div>
                <div className="text-2xl font-bold tabular-nums">{cc}<span className="text-sm font-normal" style={{opacity:"var(--dim)"}}> / {workout.length}</span></div>
                <div className="h-1.5 rounded-full mt-2 overflow-hidden" style={{background:"var(--progress-track)"}}>
                  <div className="h-full rounded-full bg-emerald-500 transition-all duration-700" style={{width:`${workoutPct}%`}}/>
                </div>
                <div className="text-[10px] mt-1" style={{opacity:"var(--faint)"}}>
                  {dayPlan.type === "Rest" ? "Rest Day" : `${dayPlan.type} Day`}
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl" style={{background:"var(--card-alt)", border:"1px solid var(--border)"}}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-blue-400"><I.Droplet/></span>
                  <span className="text-[10px] uppercase tracking-wider font-medium" style={{opacity:"var(--dim)"}}>Water Intake</span>
                </div>
                <span className="text-[11px] tabular-nums text-blue-400 font-semibold">{water} / {goals.waterGlasses} glasses</span>
              </div>
              {/* Water glasses visual */}
              <div className="flex gap-1.5 mb-3 flex-wrap">
                {Array.from({ length: goals.waterGlasses }).map((_, i) => (
                  <div key={i} className={`w-7 h-9 rounded-lg border-2 transition-all duration-300 flex items-center justify-end flex-col overflow-hidden ${
                    i < water ? "border-blue-400/50" : ""
                  }`} style={i >= water ? {borderColor:"var(--border)"} : {}}>
                    {i < water && (
                      <div className="w-full bg-blue-400/30 rounded-b-sm transition-all duration-500" style={{height:"100%"}}/>
                    )}
                  </div>
                ))}
              </div>
              <div className="flex items-center gap-2">
                <button onClick={()=>addWater(-1)} disabled={water<=0}
              ...
              <div className="text-[10px] text-center mt-2 tabular-nums" style={{opacity:"var(--faint)"}}>{water * 250}ml / {goals.waterGlasses * 250}ml</div>
            </div>
            ...
            <div className="p-4 rounded-2xl" style={{background:"var(--card-alt)", border:"1px solid var(--border)"}}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-purple-400"><I.Scale/></span>
                  <span className="text-[10px] uppercase tracking-wider font-medium" style={{opacity:"var(--dim)"}}>Body Weight</span>
                </div>
                {latestWeight && (
                  <div className="flex items-center gap-1.5">
                    {weightDiff && weightDiff !== "0.0" && (
                      <span className={`flex items-center gap-0.5 text-[11px] font-semibold ${parseFloat(weightDiff) > 0 ? "text-red-400" : "text-emerald-400"}`}>
                        {parseFloat(weightDiff) > 0 ? <I.TrendUp/> : <I.TrendDown/>}
              ...
              <div className="flex gap-2 mb-3">
                <input
                  type="number"
                  step="0.1"
                  placeholder={todayWeight ? String(todayWeight) : "Enter weight (kg)"}
                  value={weightInput}
                  onChange={e => setWeightInput(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && logWeight()}
                  className="flex-1 rounded-xl px-3 py-2.5 text-sm outline-none transition-colors"
                  style={{background:"var(--input-bg)", color:"var(--input-text)", border:"1px solid var(--input-border)", caretColor:"var(--input-caret)"}}
                />
              ...
              {recentWeights.length > 1 && (
                <div>
                  <div className="text-[9px] uppercase tracking-wider mb-2" style={{opacity:"var(--faint)"}}>Last {recentWeights.length} entries</div>
                      ...
                      return recentWeights.map((entry, i) => {
                        ...
                        return (
                          <div key={i} className="flex-1 flex flex-col items-center gap-1">
                            <span className="text-[8px] tabular-nums" style={{opacity:"var(--dim)"}}>{entry.weight}</span>
                            <div className="w-full rounded-t-md bg-purple-400/30 transition-all duration-500" style={{height:`${Math.max(h, 8)}%`}}/>
                            <span className="text-[7px] tabular-nums" style={{opacity:"var(--faint)"}}>{entry.date.slice(5)}</span>
                          </div>
                        );
                      });
                    })()}
              ...
              {todayWeight && (
                <div className="text-center mt-2">
                  <span className="text-lg font-bold text-purple-400 tabular-nums">{todayWeight} kg</span>
                  <span className="text-[10px] ml-1" style={{opacity:"var(--faint)"}}>today</span>
                </div>
              )}
            </div>
            ...
            <div className="p-4 rounded-2xl" style={{background:"var(--card-alt)", border:"1px solid var(--border)"}}>
              <div className="flex items-center gap-2 mb-3">
                <I.Fire/>
                <span className="text-[10px] uppercase tracking-wider font-medium" style={{opacity:"var(--dim)"}}>Meals Logged</span>
                <span className="ml-auto text-[11px] px-2 py-0.5 rounded-full text-emerald-400 font-semibold tabular-nums" style={{background:"var(--badge-bg)"}}>{ml} / 4</span>
              </div>
              <div className="space-y-1.5">
                {slots.map(s => {
                  const sv = dd[s.key];
                  return (
                    <div key={s.key} className="flex items-center justify-between py-1.5 px-3 rounded-lg" style={{background: sv ? "var(--card)" : "transparent"}}>
                      <div className="flex items-center gap-2">
                        <span style={{opacity: sv ? 1 : "var(--faint)"}}>{s.icon}</span>
                        <span className="text-[12px]" style={{opacity: sv ? "var(--muted)" : "var(--faint)"}}>{s.label}</span>
                      </div>
                      {sv ? (
                        <span className="text-[11px] tabular-nums text-emerald-400 font-semibold">{Math.round(sv.macros.calories)} kcal</span>
                      ) : (
                        <span className="text-[10px]" style={{opacity:"var(--faint)"}}>Not logged</span>
                      )}
                    </div>
                  );
                })}
            ...
            {macros.calories > 0 && (
              <div className="p-4 rounded-2xl" style={{background:"var(--card-alt)", border:"1px solid var(--border)"}}>
                <div className="text-[10px] uppercase tracking-widest mb-3 font-medium" style={{opacity:"var(--faint)"}}>Macro Progress</div>
      ...
      {/* ═══ GOALS MODAL ═══ */}
      {showGoals && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={()=>setShowGoals(false)}>
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm"/>
          <div className="relative w-full max-w-sm rounded-3xl p-6 animate-slideDown"
            style={{background:"var(--bg-body)", border:"1px solid var(--border)"}}
            onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                <span className="text-amber-400"><I.Target/></span>
                <h2 className="text-lg font-bold">Custom Goals</h2>
              </div>
              <button onClick={()=>setShowGoals(false)} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{background:"var(--subtle)"}}>
                <I.X/>
              </button>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                {key:"protein", label:"Protein", unit:"g", color:"#ef4444"},
                {key:"carbs", label:"Carbs", unit:"g", color:"#3b82f6"},
                {key:"fat", label:"Fat", unit:"g", color:"#a855f7"},
                {key:"fiber", label:"Fiber", unit:"g", color:"#22c55e"},
                {key:"waterGlasses", label:"Water", unit:"glasses", color:"#38bdf8"},
              ].map(g=>(
                <div key={g.key}>
                  <label className="text-[11px] uppercase tracking-wider font-medium flex items-center gap-2 mb-1" style={{opacity:"var(--dim)"}}>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={e=>setGoalDraft(p=>({...p,[g.key]:Number(e.target.value)}))}
                      className="flex-1 rounded-xl px-3 py-2.5 text-sm outline-none transition-colors"
                      style={{background:"var(--input-bg)", color:"var(--input-text)", border:"1px solid var(--input-border)", caretColor:"var(--input-caret)"}}
                    />
                    <span className="text-[11px] w-12" style={{opacity:"var(--faint)"}}>{g.unit}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={()=>{setGoalDraft(DEFAULT_GOALS);}}
                className="flex-1 py-3 rounded-xl text-sm font-medium transition-all"
                style={{background:"var(--subtle)", color:"var(--text)"}}>
                Reset
              </button>
              <button onClick={saveGoalsDraft}
